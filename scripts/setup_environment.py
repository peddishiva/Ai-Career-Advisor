...
def create_backend_env():
    ...
    try:
        ...
        print_status(f"✓ Backend .env file created/updated: {env_file}", "SUCCESS")
        return True
        
    except Exception as e:
        print_status(f"✗ Error creating backend .env file: {str(e)}", "ERROR")
        return False
...
def create_frontend_env():
    ...
    try:
        ...
        print_status(f"✓ Frontend .env.local file created/updated: {env_file}", "SUCCESS")
        return True
        
    except Exception as e:
        print_status(f"✗ Error creating frontend .env.local file: {str(e)}", "ERROR")
        return False
...
def check_firebase_config():
    ...
    if service_account_path.exists():
        print_status("✓ Firebase service account key file exists", "SUCCESS")
        ...
        try:
            ...
            print_status(f"✗ Error reading Firebase config: {str(e)}", "ERROR")
            return False
    else:
        print_status("⚠ Firebase service account key file not found", "WARNING")
...
def main():
    """Main setup function"""
    print("=" * 60)
    print("🔧 AI Career Advisor - Environment Setup")
    print("=" * 60)
    ...
    if success:
        ...
        print_status("✗ Some setup steps failed. Please check the errors above.", "ERROR")
        sys.exit(1)
